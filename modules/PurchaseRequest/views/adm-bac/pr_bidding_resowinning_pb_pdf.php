...
<div class="resolution">
	...
	<div style="text-align:center">
		<strong>BAC Resolution Declaring Winning Bidder </strong>
	...
	<div style="text-align:justify">
		WHEREAS, The ITDI posted the Invitation to Bid for the <strong> <?= $model->prItemsdisplay->bid_title ?> of <?= ucwords(convertNumberToWord($description->quantity)) ?> (<?= $description->quantity ?>) <?= $model->prItemsdisplay->unit ?> <?= $description->itemexplode ?> </strong> with an Approved Budget of the Contract (ABC) of <strong> <?= NumberToWords::transformNumber('en', $description->total_cost) ?> <?= ($description->totalCostDecimal != 0 && $description->totalCostDecimal != null) ? ' pesos and ' . NumberToWords::transformNumber('en', $description->totalCostDecimal) . ' centavos' : '' ?> (P <?= number_format($description->total_cost, '2') ?>)</strong> to ITDI website, the Philippine Government Electronic Procurement System (PhilGEPS), conspicuous places at the premises of the ITDI and DOST compound continuously from March 25, 2022 to April 18, 2022;
		<br><br> WHEREAS, in response to the said advertisements, <strong> <?= ucwords(convertNumberToWord($countSupplier)) ?> (<?= $countSupplier ?>) </strong> suppliers signified interest, purchased bidding documents and attended the Pre-bid Conference on April 4, 2022;
		<br><br> WHEREAS, 2 bids were submitted on April 18, 2022 during the opening of bids and passed the preliminary examination of bids using pass/fail criteria (eligibility, technical and financial requirements);
		<br><br> WHEREAS, upon careful examination, validation, verification and post qualification of all the eligibility requirements, technical specifications and financial proposal, the bid of <strong><?= $model->supplierdisplay->supplier_name  ?></strong> was evaluated to be responsive and substantially complying;
	...
	<div style="text-align: center;">
		...
		<span style=" font-size: meduim">BAC Chairperson</span>
	</div>
	...
	foreach ($member_assignatory as $list) {
		...
		echo '<td style = "padding-left:70px;padding-right:70px;padding-top: 70; text-align: center; font-family: Arial" > ' . $member_list->profile->fname . '  ' . $member_list->profile->lname . '<br> Member </span></td>';
